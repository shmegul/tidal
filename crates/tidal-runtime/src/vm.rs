...
use crate::env::{Env, Binding};
use crate::value::Value;
use tidal_ast::{Type, Program};
...
pub fn run(env: &mut Env, bc: &Bytecode) -> Result<Option<(Value, bool)>> {
    ...
    fn pop2(stack: &mut Vec<Value>) -> Result<(Value, Value)> {
        let b = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
        let a = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
        Ok((a, b))
    }
    #[inline(always)]
    fn pop2_int(stack: &mut Vec<Value>) -> Result<(i64, i64)> {
        let b = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
        let a = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
        if let (Value::Int(x), Value::Int(y)) = (a, b) { Ok((x, y)) } else { Err(Error::runtime("Type mismatch: expected Int operands")) }
    }
    ...
    fn arith_add(stack: &mut Vec<Value>, static_typed: bool) -> Result<()> {
        if static_typed {
            ...
            let (a, b) = pop2(stack)?;
            match (a, b) {
                (Value::Int(x), Value::Int(y)) => { stack.push(Value::Int(x.wrapping_add(y))); Ok(()) }
                _ => Err(Error::runtime("Unsupported binary operation for given types in VM")),
            }
        }
    }
    #[inline(always)]
    fn arith_sub(stack: &mut Vec<Value>, static_typed: bool) -> Result<()> {
        if static_typed {
            ...
            let (a, b) = pop2(stack)?;
            match (a, b) {
                (Value::Int(x), Value::Int(y)) => { stack.push(Value::Int(x.wrapping_sub(y))); Ok(()) }
                _ => Err(Error::runtime("Unsupported binary operation for given types in VM")),
            }
        }
    }
    ...
    fn arith_mul(stack: &mut Vec<Value>, static_typed: bool) -> Result<()> {
        if static_typed {
            ...
            let (a, b) = pop2(stack)?;
            match (a, b) {
                (Value::Int(x), Value::Int(y)) => { stack.push(Value::Int(x.saturating_mul(y))); Ok(()) }
                _ => Err(Error::runtime("Unsupported binary operation for given types in VM")),
            }
        }
    }
    ...
    fn arith_div(stack: &mut Vec<Value>, static_typed: bool) -> Result<()> {
        if static_typed {
            let (x, y) = pop2_int(stack)?;
            if y == 0 { return Err(Error::runtime("Division by zero")); }
    ...
    fn cmp_apply(op: &Instr, stack: &mut Vec<Value>, static_typed: bool) -> Result<()> {
        if static_typed {
            ...
            let outb = match (&a, &b) {
                ...
                _ => return Err(Error::runtime("VM comparisons currently support Int operands only")),
            };
            stack.push(Value::Bool(outb));
            Ok(())
        }
    ...
    #[derive(Clone)]
    struct LoopFrame { slot: usize, /*mutable: bool,*/ current: i64, end: i64, step: i64, body_start: usize, body_end: usize }
    let mut loops: Vec<LoopFrame> = Vec::with_capacity(64);

    // Simple scope stack for Env exports (REPL-only)
    #[derive(Clone)]
    struct ExportedVar { name: String, slot: usize, prev: Option<Binding> }
    ...
    while ip < code.len() {
        match &code[ip] {
            Instr::LoadInt(n) => { stack.push(Value::Int(*n)); ip += 1; }
            Instr::LoadBool(b) => { stack.push(Value::Bool(*b)); ip += 1; }
            Instr::LoadConstStr(idx) => { stack.push(Value::String(bc.consts[*idx].clone())); ip += 1; }
            Instr::LoadLocal(slot) => { stack.push(locals[*slot].clone()); ip += 1; }
            Instr::StoreLocal(slot) => {
                // Enforce mutability on updates
                if !slot_mutable[*slot] {
                    let name_idx = bc.slot_name_idx[*slot];
                    let name = bc.names[name_idx].as_ref();
                    return Err(Error::runtime(format!("Cannot assign to immutable variable '{}'", name)));
                }
            ...
            Instr::Add => { arith_add(&mut stack, bc.has_static_types)?; ip += 1; }
            Instr::Sub => { arith_sub(&mut stack, bc.has_static_types)?; ip += 1; }
            Instr::Mul => { arith_mul(&mut stack, bc.has_static_types)?; ip += 1; }
            Instr::Div => { arith_div(&mut stack, bc.has_static_types)?; ip += 1; }
            Instr::Rem => { arith_rem(&mut stack, bc.has_static_types)?; ip += 1; }
            ...
            Instr::JumpIfFalse(target) => {
                ...
                let cond = match v {
                    Value::Bool(b) => b,
                    other => return Err(Error::runtime(format!("If condition must be Bool at runtime (got {})", other))),
                };
                if !cond { ip = *target; } else { ip += 1; }
            }
            Instr::CastTo(ty) => {
                let v = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
            ...
            Instr::Range => {
                let right = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
                let left = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
                match (left, right) {
            ...
            Instr::LetLocal { slot, mutable } => {
                ...
                if !slot_exported[*slot] {
                    ...
                    if env.should_export_vm_locals() {
                        ...
                        let ty = if let Some(t) = &slot_type[*slot] { t.clone() } else { let t = env.infer_type_public(&v); slot_type[*slot] = Some(t.clone()); t };
                        let prev = env.get_binding_cloned_public(&name);
                        let binding = Binding { mutable: *mutable, ty, value: v.clone() };
                        env.insert_binding_public(name.clone(), binding);
                        if let Some(frame) = scope_stack.last_mut() {
                            frame.push(ExportedVar { name, slot: *slot, prev });
                        }
                    }
                } else {
            ...
            Instr::ForStart { slot, mutable, body_len } => {
                let it = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
                let (start, end) = match it { Value::Range(a, b) => (a, b), other => return Err(Error::runtime(format!("'for' expects Iterator, got {}", other))) };
                ...
                if (step > 0 && start > end) || (step < 0 && start < end) {
                    ...
                    if env.should_export_vm_locals() {
                        ...
                        let ty = if let Some(t) = &slot_type[*slot] { t.clone() } else { let t = env.infer_type_public(&v); slot_type[*slot] = Some(t.clone()); t };
                        let prev = env.get_binding_cloned_public(&name);
                        let binding = Binding { mutable: *mutable, ty, value: v };
                        env.insert_binding_public(name.clone(), binding);
                        if let Some(frame) = scope_stack.last_mut() {
                            frame.push(ExportedVar { name, slot: *slot, prev });
                        }
                    }
                    loops.push(LoopFrame { slot: *slot, current: cur, end, step, body_start, body_end });
                    ip = body_start;
                }
            }
            Instr::ForEnd => {
                let mut frame = loops.pop().ok_or_else(|| Error::runtime("internal: ForEnd without frame"))?;
                let next = frame.current.saturating_add(frame.step);
                let cont = if frame.step > 0 { next <= frame.end } else { next >= frame.end };
                if cont {
                    frame.current = next;
            ...
            Instr::Break => {
                ...
                let frame = loops.pop().ok_or_else(|| Error::runtime("'break' used outside of loop (VM)"))?;
                ip = frame.body_end + 1;
            }
            Instr::Continue => {
                // Continue: jump to ForEnd of the innermost loop to advance iteration
                let frame = loops.last().ok_or_else(|| Error::runtime("'continue' used outside of loop (VM)"))?.clone();
            ...
            Instr::Pop => { let _ = stack.pop(); ip += 1; }
            Instr::IndexLoad(idx) => {
                let base = stack.pop().ok_or_else(|| Error::runtime("stack underflow"))?;
                match base {
                    Value::Array(items) => {
                        if *idx >= items.len() { return Err(Error::runtime("Index out of bounds")); }
                    ...
                    other => return Err(Error::runtime(format!("Indexing not supported on {}", other))),
                }
                ip += 1;
            }
            ...
            Instr::IncLocal(slot) => {
                ...
                match &mut locals[*slot] {
                    ...
                    other => {
                        return Err(Error::runtime(format!("IncLocal expects Int local, got {}", other)));
                    }
                }
            }
        }
