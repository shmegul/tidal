...
#![allow(dead_code, clippy::collapsible_if, clippy::collapsible_else_if, clippy::borrow_deref_ref, clippy::ptr_arg, clippy::get_first, clippy::only_used_in_recursion, clippy::unnecessary_cast, clippy::assign_op_pattern)]
...
impl Env {
    pub fn new() -> Self { Self { map: HashMap::new(), echo_inner: false, loop_depth: 0, break_now: false, continue_now: false, loop_cache: HashMap::new(), loop_cache_active: false, cast_cache: HashMap::new(), cast_cache_active: false } }

    pub fn set_echo_inner(&mut self, on: bool) { self.echo_inner = on; }

    /// Should VM export locals to Env? We use REPL's echo_inner flag as a proxy: true in REPL, false in CLI.
    pub fn should_export_vm_locals(&self) -> bool { self.echo_inner }
    ...
    fn loop_cache_insert_if_active(&mut self, name: &str, binding: &Binding) {
        if self.loop_cache_active && !binding.mutable {
            self.loop_cache.insert(name.to_string(), binding.value.clone());
        }
    }
    ...
    pub fn exec_program(&mut self, program: Program) -> Result<Option<(Value, bool)>> {
        ...
        match crate::vm::compile(&program) {
            crate::vm::CompileOutcome::Bytecode(code) => {
                crate::vm::run(self, &code)
            }
            crate::vm::CompileOutcome::Unsupported => {
                panic!("VM Unsupported: expand VM coverage or avoid unsupported constructs. No AST fallback as requested.");
            }
        }
    }
    ...
    fn exec_stmt(&mut self, stmt: Stmt) -> Result<(Value, bool)> {
        match stmt {
            Stmt::Let { mutable, name, ty, expr } => {
                ...
                let binding = Binding { mutable, ty: final_ty.clone(), value: val.clone() };
            ...
            Stmt::ExprStmt(expr) => {
                ...
                if let Expr::If { branches, else_branch } = expr.clone() {
            ...
            Stmt::Break => {
                if self.loop_depth == 0 { return Err(Error::runtime("E1001: 'break' used outside of loop. Hint: place 'break' inside for/while/loop.")); }
                self.break_now = true;
                Ok((Value::Int(0), false))
            }
            Stmt::Continue => {
                if self.loop_depth == 0 { return Err(Error::runtime("E1002: 'continue' used outside of loop. Hint: place 'continue' inside for/while/loop.")); }
                self.continue_now = true;
                Ok((Value::Int(0), false))
            }
            Stmt::AssignIndex { name, index, expr } => {
                let rhs = self.eval_expr(expr)?;
                let b = self.map.get_mut(&name).ok_or_else(|| Error::runtime(format!("Unknown variable '{}'", name)))?;
                ...
                let res = match &mut b.value {
                    Value::Array(items) => {
                        if index >= items.len() { return Err(Error::runtime("Index out of bounds")); }
                    ...
                    _ => return Err(Error::runtime("Index assignment is only supported for arrays and tuples")),
                };
                Ok(res)
            }
                ...
                match iterable {
                    Value::Array(items) => {
                        'outer_array: for it in items {
                            ...
                            let binding = Binding { mutable, ty, value: it.clone() };
                            self.loop_cache_insert_if_active(&name, &binding);
                            self.map.insert(name.clone(), binding);
                            let mut last_in_body: Option<Value> = None;
                            for op in &optimized_ops {
                                let (v, show) = self.exec_optimized_op(op)?;
                                if self.echo_inner && show { println!("{}", v); }
                                last_in_body = Some(v);
                                if self.break_now { self.break_now = false; // break out of this loop entirely
                                    self.loop_iter_end();
                                    break 'outer_array;
                                }
                                if self.continue_now { self.continue_now = false; break; }
                            }
                            if let Some(v) = last_in_body { last = Some(v); }
                    ...
                    Value::Range(start, end) => {
                        ...
                        'outer_iter: loop {
                            ...
                            let it = Value::Int(i);
                            let ty = self.infer_type(&it);
                            let binding = Binding { mutable, ty, value: it };
                            self.loop_cache_insert_if_active(&name, &binding);
                            self.map.insert(name.clone(), binding);
                            let mut last_in_body: Option<Value> = None;
                            for op in &optimized_ops {
                                ...
                                if self.echo_inner && show { println!("{}", v); }
                                last_in_body = Some(v);
                                if self.break_now { self.break_now = false; self.loop_iter_end(); break 'outer_iter; }
                                if self.continue_now { self.continue_now = false; break; }
                            }
                            if let Some(v) = last_in_body { last = Some(v); }
                            // end per-iteration cache
                    ...
                    other => { self.loop_depth = self.loop_depth.saturating_sub(1); return Err(Error::runtime(format!("'for' can iterate over Array or Iterator, got {}", other))); }
                }
                    ...
                    match cv {
                        Value::Bool(true) => {
                            ...
                            for s in &body {
                                ...
                                if self.echo_inner && show { println!("{}", v); }
                                last_in_body = Some(v);
                                if self.break_now { self.break_now = false; self.loop_iter_end(); break 'outer_while; }
                                if self.continue_now { self.continue_now = false; break; }
                            }
                            if let Some(v) = last_in_body { last = Some(v); }
                            // end per-iteration cache
                        ...
                        _ => { self.loop_depth = self.loop_depth.saturating_sub(1); return Err(Error::runtime("While condition must be Bool")); }
                    }
                }
                self.loop_depth = self.loop_depth.saturating_sub(1);
                Ok((last.unwrap_or(Value::Int(0)), false))
            ...
            Stmt::Loop { body } => {
                ...
                'outer_loop: loop {
                    ...
                    for s in &body {
                        ...
                        if self.break_now { self.break_now = false; self.loop_iter_end(); break 'outer_loop; }
                        if self.continue_now { self.continue_now = false; break; }
                    }
                    self.loop_iter_end();
                }
    ...
    fn apply_compound(&mut self, name: String, expr: Expr, op: Op) -> Result<Value> {
        let rhs = self.eval_expr(expr)?;
        let b = self.map.get_mut(&name).ok_or_else(|| Error::runtime(format!("Unknown variable '{}'", name)))?;
        if !b.mutable { return Err(Error::runtime(format!("Cannot assign to immutable variable '{}'", name))); }
        match (&mut b.value, rhs) {
            (Value::Int(l), Value::Int(r)) => {
                match op {
                    Op::Add => { *l = l.saturating_add(r); }
                    Op::Sub => { *l = l.saturating_sub(r); }
                    Op::Mul => { *l = l.saturating_mul(r); }
                    Op::Div => {
                        if r == 0 { return Err(Error::runtime("Division by zero")); }
                        *l = *l / r;
                    }
                    Op::Rem => {
                        if r == 0 { return Err(Error::runtime("Division by zero")); }
            ...
            _ => Err(Error::runtime("Unsupported compound assignment for given types")),
        }
    }
        ...
        match expr {
            ...
            Expr::String(s) => {
                match s.to_str() {
                    ...
                    Err(_) => Ok(Value::String(std::rc::Rc::new(s.to_string_lossy().into_owned()))),
                }
            }
            ...
            Expr::Ident(name) => {
                ...
                let b = self.map.get(name).ok_or_else(|| Error::runtime(format!("Unknown variable '{}'", name)))?;
            ...
            Expr::Array(items) => {
                ...
                for e in items { vals.push(self.eval_expr_ref(e)?); }
            ...
            Expr::Index(base, idx) => {
                ...
                match v {
                    Value::Array(arr) => {
                        if *idx >= arr.len() { return Err(Error::runtime("Index out of bounds")); }
                        Ok(arr[*idx].clone())
                    }
                    Value::Tuple(tup) => {
                        if *idx >= tup.len() { return Err(Error::runtime("Index out of bounds")); }
                        Ok(tup[*idx].clone())
                    }
                    _ => Err(Error::runtime("Indexing is supported only for arrays and tuples")),
                }
            }
            Expr::Binary(l, op, r) => {
                ...
                match (lv, rv, op.clone()) {
                    ...
                    (Value::Int(a), Value::Int(b), Op::Div) => {
                        if b == 0 { Err(Error::runtime("Division by zero")) } else { Ok(Value::Int(a / b)) }
                    }
                    (Value::Int(a), Value::Int(b), Op::Rem) => {
                        if b == 0 { Err(Error::runtime("Division by zero")) } else { Ok(Value::Int(a % b)) }
                    }
                    (Value::Int(a), Value::Int(b), Op::Pow) => {
                        if b < 0 { return Err(Error::runtime("Negative exponent not supported for Int")); }
                        let mut res: i64 = 1;
                        let mut base = a;
                        let mut exp = b as u64;
                        // fast pow by squaring with saturating mul
                        while exp > 0 {
                            if (exp & 1) == 1 { res = res.saturating_mul(base); }
                            exp >>= 1;
                    ...
                    (Value::Int(a), Value::Int(b), Op::Shl) => {
                        let s = if b < 0 { return Err(Error::runtime("Negative shift")); } else { (b as u64).min(63) as u32 };
                    ...
                    (Value::Float(a), Value::Float(b), Op::Rem) => {
                        if b == 0.0 { Err(Error::runtime("Division by zero")) } else { Ok(Value::Float(a % b)) }
                    },
                    ...
                    (Value::Double(a), Value::Double(b), Op::Rem) => {
                        if b == 0.0 { Err(Error::runtime("Division by zero")) } else { Ok(Value::Double(a % b)) }
                    },
                    ...
                    _ => Err(Error::runtime("Unsupported binary operation for given types")),
                }
            }
            Expr::BitNot(e) => {
            ...
            Expr::If { branches, else_branch } => {
                ...
                if let Some((cond0, blk0)) = branches.get(0) {
                    match cond0 {
                        Expr::Bool(true) => { return self.eval_block_ref(blk0); }
                        Expr::Bool(false) => { /* skip */ }
                        Expr::Binary(l, Op::Eq, r) => {
                            if let (Expr::Ident(a), Expr::Ident(b)) = (&**l, &**r) {
                                if a == b { return self.eval_block_ref(blk0); }
                            }
                ...
                for (cond, blk) in branches {
                    ...
                    match cv {
                        Value::Bool(true) => { return self.eval_block_ref(blk); }
                        Value::Bool(false) => {}
                        _ => return Err(Error::runtime("If condition must be Bool")),
                    }
                }
                if let Some(blk) = else_branch { self.eval_block_ref(blk) } else { Err(Error::runtime("No matching if branch and no else")) }
            }
            Expr::Cast(e, ty) => {
                        ...
                        if let Some(b) = self.map.get(name) {
                            if !b.mutable {
                                if let Some(tag) = Self::ty_tag(ty) {
                                    ...
                                    self.cast_cache.entry(name.clone()).or_default().insert(tag, converted.clone());
                                    return Ok(converted);
                                }
                            }
                        }
    ...
    fn check_type(&self, ty: &Type, val: &Value) -> Result<()> {
        match (ty, val) {
            ...
            (Type::Array(elem_ty, n), Value::Array(items)) => {
                if *n != items.len() { return Err(Error::runtime("Array length mismatch")); }
                for it in items { self.check_type(elem_ty, it)?; }
                Ok(())
            }
            (Type::Tuple(tys), Value::Tuple(items)) => {
                if tys.len() != items.len() { return Err(Error::runtime("Tuple arity mismatch")); }
                for (t, v) in tys.iter().zip(items.iter()) { self.check_type(t, v)?; }
                Ok(())
            }
...
impl Env {
    pub fn infer_type_public(&self, val: &Value) -> Type { self.infer_type(val) }
...
impl Env {
    ...
    fn convert_value_to(&self, v: Value, ty: &Type) -> Result<Value> {
        match ty {
                ...
                match v {
                    ...
                    Value::String(s) => {
                        if let Some(n) = Self::fast_parse_i64(s.as_str()) {
                            ...
                            s.parse::<i64>().map(Value::Int).map_err(|_| Error::runtime("Cannot cast String to Int"))
                        }
                    }
                    Value::Char(c) => Ok(Value::Int(c as i64)),
                    Value::Array(_) | Value::Tuple(_) | Value::Range(_, _) => { Err(Error::runtime("Cannot cast composite value to Int")) }
                }
            }
            Type::Float => {
                match v {
                    ...
                    Value::String(s) => s.as_str().parse::<f32>().map(Value::Float).map_err(|_| Error::runtime("Cannot cast String to Float")),
                    Value::Char(c) => Ok(Value::Float((c as u32) as f32)),
                    Value::Array(_) | Value::Tuple(_) | Value::Range(_, _) => { Err(Error::runtime("Cannot cast composite value to Float")) }
                }
            }
            Type::Double => {
                match v {
                    ...
                    Value::String(s) => s.as_str().parse::<f64>().map(Value::Double).map_err(|_| Error::runtime("Cannot cast String to Double")),
                    Value::Char(c) => Ok(Value::Double((c as u32) as f64)),
                    Value::Array(_) | Value::Tuple(_) | Value::Range(_, _) => { Err(Error::runtime("Cannot cast composite value to Double")) }
                }
            }
            Type::Bool => {
                match v {
                    ...
                    Value::Bool(b) => Ok(Value::Bool(b)),
                    Value::String(s) => match s.as_str() { "true" => Ok(Value::Bool(true)), "false" => Ok(Value::Bool(false)), _ => Err(Error::runtime("Cannot cast String to Bool")) },
                    Value::Char(c) => Ok(Value::Bool(c != '\u{0}')),
                    Value::Array(_) | Value::Tuple(_) | Value::Range(_, _) => { Err(Error::runtime("Cannot cast composite value to Bool")) }
                }
            }
            Type::String => {
                match v {
            ...
            Type::Range => {
                Err(Error::runtime("Cannot cast to Iterator"))
            }
            Type::Char => {
                match v {
                    ...
                    Value::Bool(b) => Ok(if b { Value::Char('\u{1}') } else { Value::Char('\u{0}') }),
                    Value::String(s) => {
                    ...
                    Value::Array(_) | Value::Tuple(_) | Value::Range(_, _) => { Err(Error::runtime("Cannot cast composite value to Char")) }
                }
            }
    ...
    fn eval_if_optional(&mut self, branches: Vec<(Expr, Vec<Stmt>)>, else_branch: Option<Vec<Stmt>>) -> Result<Option<Value>> {
        for (cond, blk) in branches {
            let cv = self.eval_expr(cond)?;
            match cv {
                Value::Bool(true) => { return Ok(Some(self.eval_block(blk)?)); }
                Value::Bool(false) => {}
                _ => return Err(Error::runtime("If condition must be Bool")),
            }
        }
        if let Some(blk) = else_branch { Ok(Some(self.eval_block(blk)?)) } else { Ok(None) }
    }
    ...
    pub fn convert_value_to_public(&self, v: Value, ty: &Type) -> Result<Value> { self.convert_value_to(v, ty) }
}
...
impl Env {
    pub fn exec_stmt_ref(&mut self, stmt: &Stmt) -> Result<(Value, bool)> {
        match stmt {
            Stmt::Let { mutable, name, ty, expr } => {
                ...
                let binding = Binding { mutable: *mutable, ty: final_ty.clone(), value: val.clone() };
                self.loop_cache_insert_if_active(name, &binding);
                self.map.insert(name.clone(), binding);
                Ok((val, false))
            }
            ...
            Stmt::AssignIndex { name, index, expr } => {
                ...
                let b = self.map.get_mut(name).ok_or_else(|| Error::runtime(format!("Unknown variable '{}'", name)))?;
                if !b.mutable { return Err(Error::runtime("Cannot assign to immutable object")); }
                let res = match &mut b.value {
                    Value::Array(items) => {
                        if *index >= items.len() { return Err(Error::runtime("Index out of bounds")); }
                        items[*index] = rhs.clone();
                ...
                match iterable {
                    Value::Array(items) => {
                        'outer_array_ref: for it in items {
                            self.loop_iter_begin();
                            let ty = self.infer_type(&it);
                            let binding = Binding { mutable: *mutable, ty, value: it.clone() };
                            self.loop_cache_insert_if_active(name, &binding);
                            self.map.insert(name.clone(), binding);
                            let mut last_in_body: Option<Value> = None;
                            for op in &optimized_ops {
                                ...
                                if self.echo_inner && show { println!("{}", v); }
                                last_in_body = Some(v);
                                if self.break_now { self.break_now = false; self.loop_iter_end(); break 'outer_array_ref; }
                                if self.continue_now { self.continue_now = false; break; }
                            }
                            if let Some(v) = last_in_body { last = Some(v); }
                            self.loop_iter_end();
                    ...
                    Value::Range(start, end) => {
                        ...
                        'outer_iter_ref: loop {
                            ...
                            let it = Value::Int(i);
                            let ty = self.infer_type(&it);
                            let binding = Binding { mutable: *mutable, ty, value: it };
                            self.loop_cache_insert_if_active(name, &binding);
                            self.map.insert(name.clone(), binding);
                            let mut last_in_body: Option<Value> = None;
                            for op in &optimized_ops {
                                ...
                                if self.echo_inner && show { println!("{}", v); }
                                last_in_body = Some(v);
                                if self.break_now { self.break_now = false; self.loop_iter_end(); break 'outer_iter_ref; }
                                if self.continue_now { self.continue_now = false; break; }
                            }
                            if let Some(v) = last_in_body { last = Some(v); }
                            self.loop_iter_end();
                    ...
                    match cv {
                        Value::Bool(true) => {
                            ...
                            for s in body {
                                ...
                                if self.echo_inner && show { println!("{}", v); }
                                last_in_body = Some(v);
                                if self.break_now { self.break_now = false; self.loop_iter_end(); break 'outer_while_ref; }
                                if self.continue_now { self.continue_now = false; break; }
                            }
                            if let Some(v) = last_in_body { last = Some(v); }
                            self.loop_iter_end();
            ...
            Stmt::Loop { body } => {
                ...
                'outer_loop_ref: loop {
                    ...
                    for s in body {
                        ...
                        if self.echo_inner && show { println!("{}", v); }
                        _last_in_body = Some(v);
                        if self.break_now { self.break_now = false; self.loop_iter_end(); break 'outer_loop_ref; }
                        if self.continue_now { self.continue_now = false; break; }
                    }
                    self.loop_iter_end();
                }
    ...
    fn eval_if_optional_ref(&mut self, branches: &Vec<(Expr, Vec<Stmt>)>, else_branch: Option<&Vec<Stmt>>) -> Result<Option<Value>> {
        // Fast paths on first branch
        if let Some((cond0, blk0)) = branches.get(0) {
            match cond0 {
                Expr::Bool(true) => { return Ok(Some(self.eval_block_ref(blk0)?)); }
                Expr::Bool(false) => { /* skip */ }
                Expr::Binary(l, Op::Eq, r) => {
                    if let (Expr::Ident(a), Expr::Ident(b)) = (&**l, &**r) {
                        if a == b { return Ok(Some(self.eval_block_ref(blk0)?)); }
                    }
                    if let (Expr::Int(a), Expr::Int(b)) = (&**l, &**r) {
                        if a == b { return Ok(Some(self.eval_block_ref(blk0)?)); }
                    }
        ...
        for (cond, blk) in branches.iter() {
            let cv = self.eval_expr_ref(cond)?;
            match cv {
                Value::Bool(true) => { return Ok(Some(self.eval_block_ref(blk)?)); }
                Value::Bool(false) => {}
                _ => return Err(Error::runtime("If condition must be Bool")),
            }
        }
        if let Some(blk) = else_branch { Ok(Some(self.eval_block_ref(blk)?)) } else { Ok(None) }
    }
}
