...
pub fn compile(program: &Program) -> CompileOutcome {
    ...
    fn cstring_to_string(cs: &std::ffi::CString) -> String {
        match cs.to_str() {
            Ok(st) => st.to_owned(),
            Err(_) => cs.to_string_lossy().into_owned(),
        }
    }
    ...
    fn compile_expr_inner(
        ...
        match expr {
            Expr::Int(n) => {
                out.push(Instr::LoadInt(*n as i64));
                Ok(())
            }
            Expr::String(cs) => {
                ...
                let idx = consts
                    .iter()
                    .position(|rc| rc.as_str() == s)
                    .unwrap_or_else(|| {
                        consts.push(Rc::new(s.clone()));
                        consts.len() - 1
                    });
            ...
            Expr::Binary(l, op, r) => {
                ...
                match (&**l, &**r, op) {
                    (Expr::Int(a), Expr::Int(b), Op::Add) => {
                        out.push(Instr::LoadInt((*a as i64) + (*b as i64)));
                        return Ok(());
                    }
                    (Expr::Int(a), Expr::Int(b), Op::Sub) => {
                        out.push(Instr::LoadInt((*a as i64) - (*b as i64)));
                        return Ok(());
                    }
                    (Expr::Int(a), Expr::Int(b), Op::Mul) => {
                        out.push(Instr::LoadInt((*a as i64) * (*b as i64)));
                        return Ok(());
                    }
                    ...
                    (Expr::Int(a), Expr::Int(b), Op::Eq) => {
                        out.push(Instr::LoadBool((*a as i64) == (*b as i64)));
                        return Ok(());
                    }
                    (Expr::Int(a), Expr::Int(b), Op::Ne) => {
                        out.push(Instr::LoadBool((*a as i64) != (*b as i64)));
                        return Ok(());
                    }
                    (Expr::Int(a), Expr::Int(b), Op::Lt) => {
                        out.push(Instr::LoadBool((*a as i64) < (*b as i64)));
                        return Ok(());
                    }
                    (Expr::Int(a), Expr::Int(b), Op::Gt) => {
                        out.push(Instr::LoadBool((*a as i64) > (*b as i64)));
                        return Ok(());
                    }
                    (Expr::Int(a), Expr::Int(b), Op::Le) => {
                        out.push(Instr::LoadBool((*a as i64) <= (*b as i64)));
                        return Ok(());
                    }
                    (Expr::Int(a), Expr::Int(b), Op::Ge) => {
                        out.push(Instr::LoadBool((*a as i64) >= (*b as i64)));
                        return Ok(());
                    }
                    (Expr::Bool(a), Expr::Bool(b), Op::Eq) => {
                        out.push(Instr::LoadBool(*a == *b));
                        return Ok(());
                    }
                    (Expr::Bool(a), Expr::Bool(b), Op::Ne) => {
                        out.push(Instr::LoadBool(*a != *b));
                        return Ok(());
                    }
                    (Expr::Ident(a), Expr::Ident(b), Op::Eq) => {
                        out.push(Instr::LoadBool(a == b));
                        return Ok(());
                    }
                    (Expr::Ident(a), Expr::Ident(b), Op::Ne) => {
                        out.push(Instr::LoadBool(a != b));
                        return Ok(());
                    }
                    _ => {}
                ...
                match op {
                    ...
                    Op::Mul => {
                        if matches!(&**l, Expr::Int(a) if *a == 0)
                            || matches!(&**r, Expr::Int(b) if *b == 0)
                        {
                            out.push(Instr::LoadInt(0));
                            return Ok(());
                        }
                ...
                match op {
                    Op::And => {
                        ...
                        let end_pos = out.len();
                        if let Instr::JumpIfFalse(t) = &mut out[jf1_pos] {
                            *t = false_pos;
                        } else {
                            return Err(());
                        }
                        if let Instr::JumpIfFalse(t) = &mut out[jf2_pos] {
                            *t = false_pos;
                        } else {
                            return Err(());
                        }
                        if let Instr::Jump(t) = &mut out[jend_pos] {
                            *t = end_pos;
                        } else {
                            return Err(());
                        }
                    ...
                    Op::Or => {
                        ...
                        out.push(Instr::LoadBool(false));
                        let end_pos = out.len();
                        if let Instr::JumpIfFalse(t) = &mut out[jf_left_pos] {
                            *t = eval_right_pos;
                        } else {
                            return Err(());
                        }
                        if let Instr::JumpIfFalse(t) = &mut out[jf_right_pos] {
                            *t = false_pos;
                        } else {
                            return Err(());
                        }
                        if let Instr::Jump(t) = &mut out[jend1_pos] {
                            *t = end_pos;
                        } else {
                            return Err(());
                        }
                        if let Instr::Jump(t) = &mut out[jend2_pos] {
                            *t = end_pos;
                        } else {
                            return Err(());
                        }
            ...
            Expr::If {
                ...
                for (idx, (cond, blk)) in branches.iter().enumerate() {
                    ...
                    let jend_pos = if need_jump_to_end {
                        let p = tmp.len();
                        tmp.push(Instr::Jump(0));
                        Some(p)
                    } else {
                        None
                    };
                    patches.push(BrPatch {
                        jfalse_pos,
                        jend_pos,
                    });
                }
                ...
                for (i, patch) in patches.iter().enumerate() {
                    let next_start_rel = if i + 1 < branch_starts.len() {
                        branch_starts[i + 1]
                    } else {
                        else_start
                    };
                    if let Some(instr) = tmp.get_mut(patch.jfalse_pos) {
                        if let Instr::JumpIfFalse(t) = instr {
                            *t = base + next_start_rel;
                        }
                    }
                    if let Some(jend_pos) = patch.jend_pos {
                        if let Some(instr) = tmp.get_mut(jend_pos) {
                            if let Instr::Jump(t) = instr {
                                *t = base + end_idx;
                            }
                        }
                    }
        ...
        fn visit_stmts(stmts: &[Stmt], set: &mut HashSet<String>) {
            for s in stmts {
                match s {
                    ...
                    Stmt::AssignIndex { name, .. } => {
                        set.insert(name.clone());
                    }
                    Stmt::For { body, .. } | Stmt::While { body, .. } | Stmt::Loop { body, .. } => {
                        visit_stmts(body, set);
                    }
                    Stmt::ExprStmt(Expr::If {
                        branches,
                        else_branch,
                    }) => {
                        for (_cond, blk) in branches {
                            visit_stmts(blk, set);
                        }
        ...
        for s in body {
            match s {
                Stmt::For {
                    ...
                    if name_is_loop_var_in_nested_for(inner_body, nm) {
                        return true;
                    }
                }
                _ => {}
    ...
    fn expr_is_invariant(expr: &Expr, loop_var: &str, assigned: &HashSet<String>) -> bool {
        match expr {
            ...
            | Expr::Char(_) => true,
            Expr::Binary(l, _op, r) => {
                expr_is_invariant(l, loop_var, assigned) && expr_is_invariant(r, loop_var, assigned)
            }
            Expr::BitNot(e) | Expr::Not(e) | Expr::Cast(e, _) => {
                expr_is_invariant(e, loop_var, assigned)
            }
            Expr::Array(items) | Expr::Tuple(items) => items
                .iter()
                .all(|e| expr_is_invariant(e, loop_var, assigned)),
            Expr::Index(base, _) => expr_is_invariant(base, loop_var, assigned),
            Expr::Range(a, b) => {
                expr_is_invariant(a, loop_var, assigned) && expr_is_invariant(b, loop_var, assigned)
            }
    ...
    fn compile_stmt_with_overrides(
        ...
        match stmt {
            ...
            Stmt::ExprStmt(expr) => {
                ...
                match expr {
                    Expr::If { .. } => {
                        compile_if_expr_with_overrides(
                            expr,
                            out,
                            slots,
                            slot_of,
            ...
            Stmt::For {
                ...
                if !matches!(iter, Expr::Range(_, _)) {
                    return Err(());
                }
                    ...
                    } = stmt
                    {
                        ...
                        fn expr_refs_any(expr: &Expr, names: &HashSet<String>) -> bool {
                            match expr {
                                ...
                                Expr::Binary(l, _op, r) => {
                                    expr_refs_any(l, names) || expr_refs_any(r, names)
                                }
                                Expr::BitNot(e) | Expr::Not(e) | Expr::Cast(e, _) => {
                                    expr_refs_any(e, names)
                                }
                                Expr::Array(items) | Expr::Tuple(items) => {
                                    items.iter().any(|e| expr_refs_any(e, names))
                                }
                                Expr::Index(base, _) => expr_refs_any(base, names),
                                Expr::Range(a, b) => {
                                    expr_refs_any(a, names) || expr_refs_any(b, names)
                                }
                                Expr::Block(_)
                                | Expr::If { .. }
                                | Expr::Int(_)
                        ...
                        if expr_is_invariant(e, name.as_str(), &assigned)
                            && !expr_refs_any(e, &hoisted_prefix_names)
                        {
                            if *m {
                ...
                for (nm, _m, e) in predeclare.iter() {
                    ...
                    if need_reset {
                        reset_each_iter.insert(nm.clone());
                    }
                }
                ...
                for (orig, temp, _e) in immut_value_temps.iter() {
                    immut_temp_lookup.insert(orig.clone(), temp.clone());
                }
                ...
                while i < body.len() {
                                            ...
                                            if let Some((cond, then_blk)) = branches.first() {
                                                if let Expr::Binary(cl2, cop, cr2) = cond {
                                                    if matches!(cop, tidal_ast::ops::Op::Eq) {
                                                        if matches!(&**cl2, Expr::Ident(nm) if *nm == *rem_name)
                                                            && matches!(&**cr2, Expr::Int(z) if (*z as i64) == 0)
                                                        {
                    ...
                    } = &body[i]
                    {
                        if let Some(temp_name) = immut_temp_lookup.get(n) {
                            let temp_slot =
                                ensure_slot(temp_name, slots, slot_of, names, name_to_idx);
                ...
                for (nm_rc, prev) in alias_saves.into_iter() {
                    match prev {
                        Some(s) => {
                            slot_of.insert(nm_rc, s);
                        }
                        None => {
                            slot_of.retain(|k, _| !Rc::ptr_eq(k, &nm_rc));
                        }
                    }
                }

                // Emit loop without extra ScopeEnter/ScopeExit; ForStart/ForEnd bound the scope.
                out.push(Instr::ForStart {
                    slot,
                    mutable: *mutable,
                    body_len,
                });
